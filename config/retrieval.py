...
# 있으니 "왜 사라졌나" 를 알아야 한다.
#
# **여기를 고치면 색인을 다시 맞춰야 한다.** 청크 파일이 진실이고 벡터 테이블은
# 그 사본이다. 이 표는 `chunking.load_chunks` 에서 걸리므로 BM25 와 목록에는
# 바로 듣지만, **Dense 는 LanceDB 를 보므로 그대로 나온다.** 9/9 에 그것 때문에
# 뺐다고 생각한 문서가 계속 검색됐다 — API 재시작으로는 안 고쳐진다.
#
#     python src/lance_store.py --sync
#
# 크론(`docker/refresh.sh`)이 하루 다섯 번 같은 걸 하므로 놔둬도 몇 시간 안에
# 맞춰지지만, 고친 직후에 확인하려면 손으로 한 번 돌린다.
REPLACED_DOCS = {
    "R26BK01719775-0": (
