...
def sync_docs(name, chunks, embedder, verbose=True):
    ...
    **하루치에 비례**한다.
    ...
    본문이 갈린 문서는 doc_id 가 같으므로 지우고 다시 넣는다. 지우지 않고
    더하면 같은 공고의 옛 청크와 새 청크가 함께 검색된다.

    **차수가 오른 경우는 여기서 안 걸린다 (9/9 정정).** 예전 이 자리에
    "차수가 올라 본문이 갈린 경우는 doc_id 가 같으므로" 라고 적어 뒀는데
    틀렸다 — `doc_id` 는 `공고번호-차수` 라 차수가 오르면 **다른 doc_id** 다.
    그래서 옛 차수가 지워지지 않고 그대로 남는다. 실측(공고 320건): 차수가
    여럿인 공고 10건, 그중 9건은 본문 유사도 1.000 — 마감만 미룬 것이다.
    거르는 자리는 여기가 아니라 `chunking.drop_stale_revisions` 다. 그쪽에
    걸어야 검색·목록·평가가 같은 코퍼스를 본다.
