...
def retrieve(
    ...
    # **용어추가(AddKeywords) — 뺐다가 되살렸다 (9/10).** 두 번 뒤집힌 판단이라
    # 과정을 남긴다.
    #
    # ① 처음엔 이득처럼 보였다. 가중 MRR +0.008. 그런데 그때 잰 노이즈 폭이
    #    ±0.015 였다. 성적으로는 못 가른다고 봤다.
    ...
    #        `자격`·`벌금`          한 번도 안 걸림
    #
    #    넷 중 셋이 헛 낱말이다 → 메커니즘이 없다고 보고 뺐다.
    # ③ **틀린 계산이었다.** 84%는 *잡음의 양*이다. 순이득 = 이득 − 잡음인데
    #    잡음만 재고 이득은 안 쟀다.
    # ④ 그 뒤 `chunking.load_chunks` 에 캐시를 붙이면서 노이즈의 원인이
    #    드러났다. 요청마다 Document 를 새로 만들어 동점 처리가 흔들린 것이다.
    #    캐시를 붙인 뒤 2단계 `compare_retrieval` 을 두 번 돌렸더니 **모든 칸이
    #    한 글자도 안 틀리고 같았다.** 노이즈가 0 이다.
    # ⑤ 그 위에서 다시 읽으면:
    #
    #        의역 107문항   적중률 0.943 → 0.962   MRR 0.897 → 0.911
    #        배점·요구사항    변화 없음 (1.000 / 0.972 — 천장이라 올라갈 자리 없음)
    #
    #    +2문항이고, 두 번 다 똑같이 나온다. **재현되는 순이득이다.**
    #
    # 값: 2단계가 질문당 ~24% 느려진다(질문을 두 벌 돌리니까). +2문항과 맞바꿨다.
    # 한계: 사전을 코퍼스에서 뽑지 않고 짐작으로 썼다. 뽑아 쓰려면 뽑을 때 쓴
    # 문항과 잴 때 쓴 문항을 나눠야 한다. 1단계에는 안 붙였다 — 거기선 안 쟀다.
    pipeline = Pipeline([
        AddKeywords(),
        Hybrid(
            [
